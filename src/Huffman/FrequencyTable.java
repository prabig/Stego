...
/**
 * Measure the frequencies of symbols in the stream and create a table of those frequencies. This will be used to create the Huffman tree.
 * Build an optimal tree for up to 2^32 symbols.
 *
 *
 */
public class FrequencyTable implements Serializable {
	...
	public CodeTree buildTree() {
		...
		for (int i = 0; i < frequencies.length; i++) {
			if (frequencies[i] > 0)
				pqueue.add(new NodeWithFrequency(new SymbolNode(i), i, frequencies[i]));
		}
		...
		for (int i = 0; i < frequencies.length && pqueue.size() < 2; i++) {
			if (i >= frequencies.length || frequencies[i] == 0)
				pqueue.add(new NodeWithFrequency(new SymbolNode(i), i, 0));
		}
		if (pqueue.size() < 2)
