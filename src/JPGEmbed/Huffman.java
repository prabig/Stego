...
class Huffman {
	...
	public Vector bits;
	public Vector val;
	public boolean letPrint = true;
	public int letPrintCount = 0;
	 ...
	 */
	public Huffman(int Width, int Height) {/////////////////Huffman defined here
	...
	public void HuffmanBlockEncoder(BufferedOutputStream outStream,
		...
		bufferIt(outStream, DC_matrix[DCcode][nbits][0],
		...
		for (k = 1; k < 64; k++) {
			if ((temp = zigzag[jpegNaturalOrder[k]]) == 0) {				
				...
				while (r > 15) {
					bufferIt(outStream, AC_matrix[ACcode][0xF0][0],
				...
				i = (r << 4) + nbits;
				bufferIt(outStream, AC_matrix[ACcode][i][0],
		...
		if (r > 0) {
			bufferIt(outStream, AC_matrix[ACcode][0][0],
