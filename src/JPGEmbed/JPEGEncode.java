...
public class JPEGEncode extends Frame {
	Thread runner;
	...
	public JPEGEncode(Image image, int quality, OutputStream out, Payload payload) {
	...
	public void CompressEmbed(IntBuffer dctCoefficients, byte[] quantTables) {
		try {
			WriteHeaders(outStream, quantTables);
		} catch (IOException e1) {
			// TODO Auto-generated catch block
			e1.printStackTrace();
		}
		WriteCompressedData(outStream, dctCoefficients);
		WriteEOI(outStream);
		try {
			outStream.flush();
		} catch (IOException e) {
...
///////////////////////////////////////////////////
		...
		MinBlockWidth = ((imageWidth % 8 != 0) ? (int) (Math
				.floor(imageWidth / 8d) + 1) * 8 : imageWidth);
		MinBlockHeight = ((imageHeight % 8 != 0) ? (int) (Math
				.floor(imageHeight / 8d) + 1) * 8 : imageHeight);
					...
					for (i = 0; i < JpegObj.VsampFactor[comp]; i++) {
						for (j = 0; j < JpegObj.HsampFactor[comp]; j++) {
							...
							dctArray3 = dct.quantizeBlock(dctArray2,
									JpegObj.QtableNumber[comp], payload);
							// }
							// else {
...
//							}
		...
		System.out.print(dct.counterBits);
		Huf.flushBuffer(outStream);
	}

	public void WriteCompressedData(BufferedOutputStream outStream, IntBuffer dctCoefficients) {
		int i, j, r, c, a, b;
		int comp, xpos, ypos, xblockoffset, yblockoffset;
		float inputArray[][];
		float dctArray1[][] = new float[8][8];
				...
				for (comp = 0; comp < JpegObj.NumberOfComponents; comp++) {
					...
					for (i = 0; i < JpegObj.VsampFactor[comp]; i++) {
						for (j = 0; j < JpegObj.HsampFactor[comp]; j++) {
							...
							try {
							dctCoefficients.get(dctArray3); //put 64 ints intp dctArray3 from buffer
							}
...
//							}
	...
	public void WriteHeaders(BufferedOutputStream out, byte[] quantTables) throws IOException {
		int i, j, index, offset, length;
		int tempArray[];
...
//		}
		...
		out.write(quantTables);
		

		// Start of Frame Header
		byte SOF[] = new byte[10+3*JpegObj.NumberOfComponents];//new byte[10];
		SOF[0] = (byte) 0xFF;
		SOF[1] = (byte) 0xC0;
		SOF[2] = (byte) 0x00;
		SOF[3] = (byte) 17;
		...
		// Start of Scan Header
		byte SOS[] = new byte[8+2*JpegObj.NumberOfComponents];//new byte[14];
		SOS[0] = (byte) 0xFF;
		SOS[1] = (byte) 0xDA;
		SOS[2] = (byte) 0x00;
		SOS[3] = (byte) 12;
