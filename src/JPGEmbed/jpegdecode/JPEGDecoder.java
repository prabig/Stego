...
public class JPEGDecoder{
	private int height;     
	boolean letPrint = true;
	public BufferedOutputStream dctCoeff;
	...
	public JPEGDecoder(BufferedOutputStream s) {
		this.dctCoeff = s;
	}
	...
	class HuffmanTable{
		...
		int get(InputStream in) throws Exception{
			...
			Lh=get16(in);    count+=2; //get table size
			while(count<Lh){
				temp=get8(in);  count++; //get table id
				t=temp&0x0F; //get ID for 4 right most bits
				if(t>3){
					error("ERROR: Huffman table ID > 3");
				}
				c=temp>>4; //get class from 4 left most bits
				if(c>2){
					error("ERROR: Huffman table [Table class > 2 ]");
				}
				Th[t]=1; Tc[t][c]=1; //if Table ID t is present make 1, and class=1
				for(i=0; i<16; i++){
					L[t][c][i]=get8(in);  count++; //fill in lengths
				}
				for(i=0; i<16; i++) //iterate through bit lengths
					for(j=0; j<L[t][c][i]; j++){ //iterate through total number of values in ith bit-length
						if(count>Lh){
							error("ERROR: Huffman table format error [count>Lh]");
						}
						V[t][c][i][j]=get8(in); count++; //add to table
					}
			}
			if(count!=Lh){
			...
			for(i=0; i<4; i++)
				for(j=0; j<2; j++)
					if(Tc[i][j]!=0){
						Build_HuffTab(HuffTab[i][j],L[i][j],V[i][j]); //build tables
					}
			return 1;
		}
	}
  ...
  (2) (Code length) << 8 | HuffVal
	...
	private int HuffmanValue(int table[],int temp[], int index[], InputStream in) throws Exception{
		...
		if(index[0]<8){
			...
			temp[0] |= input;   //<---------------------right here is the compressed huffman value
		} else index[0] -= 8;
	...
	private int getn(InputStream in, int n, int temp[], int index[]) throws Exception{
		...
		if( result < (one<<(n-1)) )
			result += (n_one << n) + 1;
		return result; //<-------------------------------THIS IS THE UNCOMPRESSED QUANTIZED DC VALUE!!
	}
  ...
  Decode MCU
			...
			int temp[], int index[]) throws Exception
	{
		...
		for(Cs=0; Cs<nComp; Cs++){
			...
			int[] tempHoldDCT = new int[64];
			for(int i=0 ; i<nBlock[Cs]; i++){ //
				for(int k=0; k<IDCT_Source.length; k++) 
					IDCT_Source[k]=0;
				value = HuffmanValue(dctab,temp,index,in); //Dehuff the DC

				
				if(value>=0xFF00) 
					return value;
				PrevDC[Cs] = IDCT_Source[0] = PrevDC[Cs] + getn(in,value,temp,index);
				tempHoldDCT[0] = PrevDC[Cs];
				IDCT_Source[0] *= qtab[0];
				for(int j=1 ; j<64 ; j++){
					value = HuffmanValue(actab,temp,index,in); //Dehuff AC 
					if(value>=0xFF00) {
						return value;//marker
					}
					j += (value>>4) ;
					if( (value&0x0F) == 0){
						if( (value>>4) == 0) {
							//Arrays.fill(tempHoldDCT, j, 64, 0);
							break ;
						}
					}else {
						
						int tN = getn(in,value&0x0F,temp,index);
						tempHoldDCT[JPGEmbed.JPEGEncode.jpegNaturalOrder[j]] = tN;
				...
				for(int ind=0; ind<64; ind++) {
					dctCoeff.write(ByteBuffer.allocate(4).putInt(tempHoldDCT[ind]).array());
				}
//				System.out.println(Arrays.toString(tempHoldDCT));
//				Thread.sleep(100);
//				for(int ind=0; ind<64; ind++) {
//					int x = this.table[ind];
//					System.out.print(IDCT_Source[x]/qTab[Cs][x]+", ");
//					Thread.sleep(25);
//					this.dctCoeff.write(IDCT_Source[ind]/qTab[Cs][ind]);
//				}
//				System.out.println();
				ScaleIDCT(DU[Cs][i]) ;
			}
	...
	public void decode(InputStream in) throws Exception{
		...
		while(current>>4 != 0x0FFC){   //SOF 0~15
			switch(current){
			case 0xFFC4:  //DHT
				HT.get(in); break; //get huffman tree
			case 0xFFCC:  //DAC
